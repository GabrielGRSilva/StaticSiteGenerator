...
def split_nodes_image(old_nodes):
    ...
    for each_old_node in old_nodes:
        ...
        if len(image_parts) == 0:
            ...
            text_parts = re.findall(r'(?:^|(?<=\)))([^!\[\]()]+)',each_old_node.text) #This regex selects everything OUTSIDE of parentheses                            #see I explanation in above function
            for i in range(max(len(text_parts), len(image_parts))):
                if i < len(text_parts):
                    print("TEXT PART:", repr(text_parts[i]))
                ...
                if i < len(image_parts):
                    new_node_list.append(TextNode(image_parts[i][0], TextType.IMAGE, image_parts[i][1]))
